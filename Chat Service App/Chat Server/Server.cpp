...
void Server::ProcessPacket(sf::TcpSocket * client, sf::Packet packet) {
	std::int8_t type; std::string username, data;
	...
	switch (type)
	{
		case (MESSAGE | REQUEST): {
			packet << (std::int8_t)(MESSAGE | OK) << username << data;
			{
				auto addrOpt = client->getRemoteAddress();
				if (addrOpt) server_network.BroadcastPacket(packet, addrOpt.value(), client->getRemotePort());
				else server_network.BroadcastPacket(packet);
			}
...
void Server::SendNotification(const std::string & message, bool to_all, sf::IpAddress exclude_address, unsigned short port) {
	sf::Packet packet;
			packet << (std::int8_t)(NOTIFICATION | OK) << "SERVER" << message;
